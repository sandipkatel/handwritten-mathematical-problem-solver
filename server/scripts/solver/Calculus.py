    ...
    if is_definite_integral:
        ...
        if integrand == sp.exp(integration_var):
            ...
            try:
                ...
                upper_value = indefinite_integral.subs(integration_var, upper_bound)
                lower_value = indefinite_integral.subs(integration_var, lower_bound)
                integral_result = upper_value - lower_value
            except Exception as e:
                print(f"Error evaluating at bounds: {e}")
                # Fallback to direct integration
                integral_result = sp.integrate(integrand, (integration_var, lower_bound, upper_bound))
...
def handle_differentiation(expr):
    ...
    plot_derivative(function, derivative_result, diff_var, order)
    result = {
        "original": sp.latex(expr),
        "type": "derivative",
    ...
    try:
        ...
        try:
            if function == sp.exp(var):
                fx = lambda x: np.exp(x)  # Handle e^x directly
            else:
                fx = sp.lambdify(var, function, 'numpy')
            
            if integral == sp.exp(var):
                fx_int = lambda x: np.exp(x)  # Handle e^x directly
            else:
...
def plot_derivative(function, derivative, var, order=1, x_range=(-5, 5), num_points=1000):
    ...
    try:
        ...
        plt.title(f"Function $f({var})$ and its Derivative $f{derivative_order}({var})$", fontsize=14)
        plt.legend(fontsize=10)
        plt.grid(True, alpha=0.3)
