...
def handle_equation(equation, solve_for=None):
    ...
    if solve_for is None:
        ...
        if len(all_symbols) > 1:
            print(f"Multiple variables found: {all_symbols}. Please specify which to solve for.")
...
def analyze_polynomial(expr, variable=None):
    ...
    if variable is None:
        ...
        if len(all_symbols) == 0:
            ...
            return {"error": "No variables found in the expression"}
        if len(all_symbols) > 1:
            print(f"Multiple variables found: {all_symbols}. Using {all_symbols[0]} for analysis.")
...
def solve_system_of_equations(equations, variables=None):
    ...
    if not solution:
        print("No solutions found.")
        result["status"] = "No solutions found"
    # elif len(solution) == 1:
    ...
    # else:
    #     print(f"Multiple solutions found ({len(solution)}):")
    #     result["status"] = "Solutions found"
    ...
    #             display(Math(sp.latex(var) + " = " + sp.latex(val)))
        ...
        for sol_dict in solution:
            ...
            result["solutions"].append(solution_entry)
...
def plot_2d_system(equations, variables, solutions):
    ...
    # If we have solutions, adjust the limits
    if solutions:
        x_vals = []
        y_vals = []
        ...
        try:
            ...
            for y_solution in y_expr:
                ...
                try:
                    ...
                    plt.plot(x[valid_indices], y[valid_indices], color=color, label=f"${sp.latex(eq)}$")
                except Exception as e:
        ...
        for sol in solutions:
            if x_var in sol and y_var in sol:
                try:
                    ...
                    plt.scatter([x_sol], [y_sol], color='green', s=100, zorder=5)
